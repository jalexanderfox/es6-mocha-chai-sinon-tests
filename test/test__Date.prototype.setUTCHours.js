	...
	describe('#prototype', function(){
		describe('#setUTCHours', function(){
			...
			it('returns a new timestamp value for the current object.', function(){
				...
				expect( theBigDay.setUTCHours(10) ).to.equal( theBigDay.getTime() );
			});
			...
			it('The values returned from the getMinutes(), getSeconds(), and getMilliseconds() methods are used if the minutesValue, secondsValue, and msValue parameters are not specified.', function(){
				...
				hoursDate.setUTCHours(2);
				expect( hoursDate.getUTCHours() ).to.equal( 2 );
			});
			...
			it('attempts to update the date information in the Date object even if a parameter is out of its expected range', function(){
				...
				overRangeHours.setUTCHours( 25 );
				expect( overRangeHours.getUTCHours() ).to.equal( 1 );
				expect( overRangeHours.getUTCDate() ).to.equal( 2 );
			});
		});
	});
});
