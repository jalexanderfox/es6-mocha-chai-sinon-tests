	...
	describe('#prototype', function(){
		describe('#toLocaleDateString', function(){
			...
			it.skip('returns a string with a language sensitive representation of the date portion of this date', function(){
				...
				expect( date.toLocaleDateString('en-US') ).to.equal( "12/19/2012" );

				// British English uses day-month-year order
				expect( date.toLocaleDateString('en-GB') ).to.equal( "20/12/2012" );

				// Korean uses year-month-day order
				expect( date.toLocaleDateString('ko-KR') ).to.equal( "2012. 12. 20." );

				// Arabic in most Arabic speaking countries uses real Arabic digits
				expect( date.toLocaleDateString('ar-EG') ).to.equal( "٢٠‏/١٢‏/٢٠١٢" );

				// for Japanese, applications may want to use the Japanese calendar,
				// where 2012 was the year 24 of the Heisei era
				expect( date.toLocaleDateString('ja-JP-u-ca-japanese') ).to.equal( "24/12/20" );

				// when requesting a language that may not be supported, such as
				// Balinese, include a fallback language, in this case Indonesian
				expect( date.toLocaleDateString(['ban', 'id']) ).to.equal( "20/12/2012" );
			...
			it.skip('accepts options for formatting', function(){
				...
				expect( date.toLocaleDateString('de-DE', options) ).to.equal( "Donnerstag, 20. Dezember 2012" );

				// an application may want to use UTC and make that visible
				options.timeZone = 'UTC';
				options.timeZoneName = 'short';
				expect( date.toLocaleDateString('en-US', options) ).to.equal( "Thursday, December 20, 2012, GMT" );
			});
		});
	});
});
